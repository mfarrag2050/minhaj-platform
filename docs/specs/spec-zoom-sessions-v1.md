...
- **M-9 — لا رابط عامّ إطلاقاً.** كل داخل مسجَّل عبر Zoom Registrant API باسم يوضع من عندنا: `«الاسم الأوّل + حرف العائلة»` من `student_profiles` (تقليل بيانات — لا اسم كامل في نظام طرف ثالث).
...
- **M-12 — الفاعل والموضوع.** إصدار التذكرة يمرّ بـ`AccessPolicy::join_role( $user_id, $session_id, $subject_student_id )` (spec-access §6). وليّ الأمر الذي يفتح لطفله: `actor_user_id` = الوصيّ، `subject_student_id` = الطفل، `role` = `participant`. هذا هو قرار 6 مطبَّقاً في صفّ قاعدة بيانات.
